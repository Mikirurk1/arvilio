...
Authentication and authorization for Arvilio. Auth is now split into **request-time authentication** (Next proxy + server session handoff) and **backend authorization** (Nest guards + service checks).
...
sequenceDiagram
  participant Browser
  participant Web as apps/campus
  ...
  API->>Browser: Set-Cookie arvilio_at arvilio_rt
...
| Cookie | Purpose | TTL |
|--------|---------|-----|
| `arvilio_at` | Campus JWT access (httpOnly) | 30 min |
| `arvilio_rt` | Campus refresh (httpOnly) | 30 days |
| `arvilio_pat` | Control Plane JWT access (httpOnly) — set only by `POST /api/auth/platform/login` | 30 min |
| `arvilio_prt` | Control Plane refresh (httpOnly) | 30 days |

Campus and Control Plane use **separate cookie names** so localhost Campus (`:4200`) and Platform (`:4300`) do not collide (cookies are host-only and ignore port). AuthGuard / session resolution prefer `arvilio_pat` when both jars are present. Impersonation still overwrites Campus `arvilio_at` only.
...
- **Control Plane:** `POST /api/auth/platform/login` — requires active `PlatformOperator`; non-operators get neutral `401` and **platform** cookies cleared. Sets `arvilio_pat` / `arvilio_prt` (not Campus `arvilio_at` / `arvilio_rt`). Throttle 5/15m. No register/OAuth/forgot on `apps/platform`. Login copy: title **Sign in**, subtitle restricted-admin only (no CLI footer in UI). Recovery: `npm run platform-operator` + `SUPER_ADMIN_CLI_SECRET` (`docs/tmp-plans/01-control-plane-secure-login.md`). Platform middleware gates pages on **`arvilio_pat` only**; **`/api/*` is excluded** so login POST is not redirected. Unauthenticated console → clean `/login` (no `?next=`). Auth is **REST**, not GraphQL.
...
- Google sign-in: `/api/auth/google`, `/api/auth/google/callback`. Web login must use a plain `<a href>` (or `window.location`) — not Next `<Link>` — because the path is a Nest rewrite/redirect; App Router RSC prefetch would fail with “Failed to fetch RSC payload”.
- **OAuth callback host:** `google.callbackUrl` must be the **browser-facing web origin** (e.g. `http://localhost:4200/api/auth/google/callback` via Next rewrite), not Nest `:3000`. Host-only cookies set on `:3000` are not sent to `:4200`, so Google can succeed (OAuthAccount updated) while the app stays logged out with no error. Defaults use `WEB_ORIGIN` / `OAUTH_PUBLIC_BASE_URL`; override with `GOOGLE_CALLBACK_URL` when the API is public on its own host. Same for Facebook/Zoom callbacks.
- Google link (logged in): `/api/auth/google/link` → callback → Profile `?tab=connections&google_linked=1` (UI already uses `window.location`)
...
- **Change password:** GraphQL `changeMyPassword(input: { currentPassword, newPassword })` → `UsersService.changeMyPassword` (min 8 chars; rejects OAuth-only accounts without `passwordHash`). Web: Profile → Account tab → modal (`AccountPanel` in `apps/campus/src/app/profile/panels.tsx`, `profile-store.changePassword`). Session user `hasPassword` from `GET /api/auth/me` / login payload — when false, UI shows linked-provider hint instead of the Change button.
...
- `apps/campus/src/proxy.ts` owns public/protected route classification (`/login`, `/forgot-password`, `/reset-password` stay public) and redirects anonymous users to a clean `/login` before render instead of relying on hydrated client redirects.
...
- Proxy also stamps the request with normalized auth headers so `apps/campus/src/app/layout.tsx` can pick the shell variant (`auth` vs `app`) and bootstrap the client auth store from server-resolved session data.
...
- **Delete account:** not exposed in Profile → Account (staff-only via admin panel). `apps/campus/src/app/admin/page.tsx` calls `confirmDialog` before `deleteUser` (SUPER_ADMIN rows not deletable in UI).
...
| Feature matrix | `lib/roles.ts` (`lib/auth/role-matrix.ts`) | `canView`, `canEdit`, `canSchedule`, `canManage` per scope |
...
5. **`useActiveUser` shim** — MockUser-shaped shell with empty defaults; identity from session; prefer profile/API stores for prefs/stats (no demo seed users)
