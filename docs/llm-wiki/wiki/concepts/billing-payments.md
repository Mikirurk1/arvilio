...
`StorageAccountingService.add(schoolId, ±bytes)` (atomic, clamps ≥0) maintains `School.storageUsedBytes`. **Materials uploads are wired:** create → `assertCanUpload` + increment (compression delta reconciled), delete → decrement. Upstream modules import the leaf barrel `@be/billing/entitlements` (no `BillingModule`) to avoid the auth↔billing barrel cycle. **Reconcile:** `StorageAccountingService.reconcile(schoolId)` recomputes used bytes from materials + lesson files + speaking audio + recordings (chat 24h attachments excluded). Admin: `POST /api/billing/entitlements/reconcile-storage`. Needed when files existed before accounting was wired (counter stays 0 while files are on disk). **Usage meter:** `EntitlementsService.getSummary` + `GET /api/billing/entitlements` → plan, seats, features, storage, **`billingManaged`** (true only when Stripe customer + subscription/plan exist — grandfathered PRO without Stripe must not show “Manage subscription”). **Layer-B (school→platform subscriptions):** `PlatformSubscriptionService` on the platform's own Stripe account (ADR-010). **Control Plane Billing** (not Settings): `/billing/rails` (PSP adapters + secrets; **default rail**, enable/disable, search/market filters, **Configured** badge, **Test connection**), `/billing/campus-plans` (default offer + country overrides; rail picker shows only **enabled + configured** rails, coerces stale railIds; price fields by `pricingMode`: Stripe → price ID and/or amount, UA rails → amount+currency only). Config: `PlatformSettings.platformBillingConfig.products.campus_subscription` + `platformBillingSecrets` + `defaultRailId`. Offer resolves from **`School.billingCountry`** (CP-only; never IP) → country override else default. Stripe checkout uses offer `stripePriceId` or `amountMinor`/`price_data`, else env `STRIPE_PRICE_*`. Non-Stripe rails: offer only until adapters ship. REST: `GET`/`PUT /api/platform/billing/rails`, `POST …/rails/:id/test`, `…/campus-subscription` (legacy `billing-rails` alias). `createCheckout` → Stripe session; webhook `applySubscriptionEvent` drives `SchoolSubscription` + `School.status`. Routes: `POST /api/platform-billing/stripe/webhook` + `POST /api/billing/subscription/checkout` (admin). **UI:** Campus `/billing`; CP Billing for rails/plans. Connect fee products reserved as `products.connect_*` (not built). **Dunning:** `TrialLifecycleService.suspendOverdueSubscriptions` (daily cron) suspends ACTIVE schools `PAST_DUE` > `DUNNING_GRACE_DAYS` (7). **Plan resolution (grandfathering):** `resolveForSchool` = paid plan if set; else `School.status==='TRIAL'`→TRIAL; else legacy ACTIVE-no-subscription → grandfathered PRO/unlimited. **Seat enforcement:** `createUserAsAdmin` creates the user's `SchoolMembership` and blocks a new STUDENT past `canAddActiveStudent` (403). **Storage enforcement wired:** materials, lessons, speaking. **Chat excluded.** **Feature-gating:** `EntitlementsService.hasFeature`/`assertFeature` + `@RequiresFeature(feature)` + `FeatureGuard`.
...
| Provider | Passes checkout currency from platform? | Practical support in Arvilio today |
|----------|----------------------------------------|--------------------------------------|
...
- Recent balance activity: shared `LessonBalanceLedgerActivity` (`apps/campus/src/components/billing/`) with human-readable kind labels (`PURCHASE`, `MANUAL_CREDIT`, `CONSUMPTION`, `REVERSAL`, `GROUP_PURCHASE`, `GROUP_MANUAL_CREDIT`, `GROUP_CONSUMPTION`, `GROUP_REVERSAL`, `GROUP_CHARGE`, `GROUP_CHARGE_REVERSAL`), icons, grouped by day, relative timestamps, and `+N lessons` / `N lessons left` copy. Helpers in `apps/campus/src/lib/billing/ledger-display.ts`.
- **Dual credit balances** ([[concepts/group-lessons]]): `StudentLessonBalance.balance` = individual prepaid credits; `groupBalance` = group per-member prepaid credits. Individual lessons consume `CONSUMPTION` (−1 from `balance`). Group `PER_MEMBER` consumes `GROUP_CONSUMPTION` (−1 from `groupBalance`). Group `FIXED_TOTAL` still uses `GROUP_CHARGE` rows with `amountMinor` (`delta: 0`, no credit bucket). Platform group per-member default price: `paymentConfig.groupLessons.defaultPriceMinor`; student override: `groupPricePerLessonMinor`. Package templates may set `creditTrack: individual | group` so checkout tops up the correct bucket. Ledger unique `(scheduledLessonId, userId, kind)`. Staff student profile: dual-track summary via `apps/campus/src/lib/billing/student-billing-tracks.ts`.
- `/payment` — when self-serve packages are enabled: **(1)** choose a package (grouped by currency when multiple ISO codes exist; currency badge on card), **(2)** order summary (“You buy …”, amount, lessons, balance after), **(3)** pay online — only providers compatible with the selected package currency (`apps/campus/src/lib/billing/checkout-display.ts`; Lemon Squeezy uses `myLessonBalance.lemonSqueezyVariantCurrency`). Hidden providers show a short “not available for {currency}” note. **Method privacy:** students only see copy for methods in their allowlist — if only `manual_invoice` is available, no “online checkout is not enabled” callout; subtitle / package CTA / aside trust line / “Or bank transfer” heading adapt. **Manual details:** each field (card number, IBAN, purpose, reference, etc.) is click-to-copy (`CopyablePaymentValue` in `payment-ui.tsx`). Bank transfer section includes “Transfer {amount} {currency}” when a package is selected. Balance hero “Your rate” only when `showPerLessonPricing` (per-lesson / both mode), in `defaultCurrency`. Footer: **trust strip** with logos for enabled online methods (`PaymentMethodLogos`) + links to public legal pages.
- Package cards show optional `description`, else fallback “N lessons · prepaid credit”.
...
- Student-facing provider copy: `apps/campus/src/app/payment/payment-page-meta.ts` (`buildPaymentMethodStudentMeta(t)`)
...
- Self-serve package cards use stronger visual hierarchy: `Starter` / `Popular choice` / `Premium`; the middle package is highlighted as `Recommended`. Multi-package picker is a **stacked list** (`packageGridInner` 1-col) beside the order summary; cards are plain `<button class="packageCard">` (not shared `Button` variants).
...
## Merchant compliance (hybrid seller profile)

- **Model:** Arvilio ships English platform legal templates; each school fills a **seller profile** (merchant of record for lesson purchases). UA locale / geo auto-switch deferred (`School.defaultLocale` / `User.locale` seams remain).
- **School fields:** `legalName`, `legalAddress`, `legalCountry` (ISO alpha-2, default UA), `supportEmail`, `supportPhone`, `mcc` (default `8299`), optional `termsOverrideMd` / `paymentRefundOverrideMd`.
- **API:** public `GET /api/school/seller-profile`; admin `PATCH /api/school/seller-profile`. UI: `System → Seller & legal`.
- **Public pages (EN):** `/offer` (catalog + non-zero prices + allowed-products note), `/legal/terms`, `/legal/payment-refund`, `/legal/contacts`; existing `/privacy`. Route policy marks these public. Templates: `apps/campus/src/content/legal/`.
- **Public catalog API:** `GET /api/school/public-offer` — packages with resolved prices + `enabledOnlineMethods` (no secrets).
- **Gate:** enabling any online PSP (`stripe` / `liqpay` / …) via `updatePaymentSettings` requires seller `legalName` + `legalAddress` + `supportEmail` (400 otherwise). Payments tab warns when profile incomplete.
- **Branding:** provider-agnostic logos under `apps/campus/public/brand/payments/` for enabled methods only — not LiqPay-only.
