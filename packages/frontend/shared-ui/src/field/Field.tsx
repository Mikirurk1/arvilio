...
import { findSelectOption, parseSelectOptionChildren } from '../advanced-select/advanced-select-options';
import type { AdvancedSelectOption } from '../advanced-select/advanced-select-options';
import { Button } from '../button/Button';
import { AdvancedSelectControl } from '../advanced-select/AdvancedSelectControl';
import { DatePickerControl } from '../date-picker/DatePickerControl';
import { TimePickerControl } from '../time-picker/TimePickerControl';
import uiStyles from '../styles/primitives.module.scss';
...
type SharedProps = {
  ...
  /** Localized aria-label when password is hidden (show action). */
  passwordToggleShowLabel?: string;
  /** Localized aria-label when password is visible (hide action). */
...
type FieldCheckboxProps = SharedProps & {
  as: 'checkbox';
  /** Bordered card layout for preference toggles. */
  variant?: 'plain' | 'card';
} & Omit<InputHTMLAttributes<HTMLInputElement>, 'type'>;

export type FieldRadioProps = SharedProps & {
  as: 'radio';
  variant?: 'plain' | 'card';
} & Omit<InputHTMLAttributes<HTMLInputElement>, 'type'>;
...
>(function Field(props, ref) {
  ...
  if (props.as === 'checkbox' || props.as === 'radio') {
    ...
    const {
      ...
      labelClassName,
      readOnly: _readOnly,
      passwordToggleShowLabel: _passwordToggleShowLabel,
      passwordToggleHideLabel: _passwordToggleHideLabel,
      ...c
    } = props;
    void as;
    void _id;
    void _formatValue;
    void _readOnly;
    void _passwordToggleShowLabel;
    void _passwordToggleHideLabel;
    const controlProps = stripNativeValidationProps(c);
    const rootClass =
      [
        uiStyles.choiceRoot,
        variant === 'card' ? uiStyles.choiceCard : null,
        rootClassName,
      ]
        .filter(Boolean)
        .join(' ') || undefined;
    const rowClass = [uiStyles.choiceRow, labelClassName].filter(Boolean).join(' ') || undefined;
    const faceClass = [
      uiStyles.choiceControl,
      kind === 'radio' ? uiStyles.choiceControlRadio : uiStyles.choiceControlCheckbox,
    ].join(' ');
    const showInlineHint = Boolean(hint) && !error;
    const hasCopy = Boolean(label) || showInlineHint;
      ...
      <div className={rootClass}>
        <label className={rowClass} htmlFor={id}>
          <input
            ...
            id={id}
            type={kind}
            className={[uiStyles.choiceInput, className].filter(Boolean).join(' ') || undefined}
            aria-invalid={props.error ? true : undefined}
            aria-describedby={describedBy}
            {...controlProps}
          />
          ...
          {hasCopy ? (
            <span className={uiStyles.choiceCopy}>
              {label ? <span className={uiStyles.choiceText}>{label}</span> : null}
              {showInlineHint ? (
                <small id={hintId} className={uiStyles.choiceHint}>
                  {hint}
                </small>
              ) : null}
            </span>
        ...
        {error ? (
          <small id={errorId} className={uiStyles.choiceError} role="alert">
            {error}
          </small>
        ) : null}
      </div>
      ...
      {isPasswordInput ? (
        <div className={uiStyles.passwordToggleWrap}>
          ...
          <button
            ...
            aria-label={
              showPassword
                ? props.passwordToggleHideLabel ?? 'Hide password'
                : props.passwordToggleShowLabel ?? 'Show password'
            }
