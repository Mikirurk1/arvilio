...
describe('MaterialAttachmentService', () => {
  let service: MaterialAttachmentService;
  const prisma = {
    libraryMaterial: {
      findUnique: jest.fn(),
    },
    libraryFileAttachment: {
      create: jest.fn(),
      findUnique: jest.fn(),
    },
  };
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new MaterialAttachmentService(
      prisma as never,
  ...
  it('getUploadDir uses MATERIAL_UPLOAD_DIR when set', () => {
    process.env.MATERIAL_UPLOAD_DIR = '/tmp/material-uploads';
    expect(service.getUploadDir()).toBe('/tmp/material-uploads');
    delete process.env.MATERIAL_UPLOAD_DIR;
  });

  it('resolveFullPath rejects path traversal', () => {
    expect(() => service.resolveFullPath('../etc/passwd')).toThrow(BadRequestException);
  });

  it('saveToDisk and openReadStream round-trip in nested layout', async () => {
    const dir = path.join(os.tmpdir(), `material-fs-${Date.now()}`);
    process.env.MATERIAL_UPLOAD_DIR = dir;
  ...
  it('createAttachment stores hierarchical storageKey', async () => {
    ...
    process.env.MATERIAL_UPLOAD_DIR = dir;
    prisma.libraryMaterial.findUnique.mockResolvedValue({ id: 'mat-1' });
  ...
  it('openReadStream throws when file missing', async () => {
    process.env.MATERIAL_UPLOAD_DIR = path.join(os.tmpdir(), 'material-missing-test');
