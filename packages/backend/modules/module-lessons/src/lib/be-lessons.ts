...
function materialKindFromDto(
  kind: 'text' | 'photo' | 'test' | 'file' | 'presentation',
): 'TEXT' | 'PHOTO' | 'TEST' | 'FILE' | 'PRESENTATION' {
  return kind.toUpperCase() as 'TEXT' | 'PHOTO' | 'TEST' | 'FILE' | 'PRESENTATION';
}

function studentResponseStatusToDto(
...
function studentResponseStatusFromDto(
...
@Module({
  ...
  providers: [LessonsService, GoogleCalendarService],
  exports: [LessonsService, GoogleCalendarService],
})
