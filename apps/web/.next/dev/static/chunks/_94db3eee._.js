...
function getDailyGoalsForUser(userId, dateKey = defaultGoalDateKey()) {
    ...
    return tiers.map((difficulty)=>{
        ...
        const pick = pool.length > 0 ? pool[hashToUInt(`${userId}|${dateKey}|${difficulty}`) % pool.length] : goalTemplates[0];
        return {
            id: `${pick.id}-${dateKey}`,
            text: pick.text,
            difficulty,
...
;
;
/** Seed vocabulary progress grouped per student user id. */ function buildSeedProfileVocabularyRows() {
...
function getProfileVocabularyForUser(userId) {
    return profileVocabularyRows.filter((r)=>r.userId === userId);
}
...
function addProfileVocabularyEntry(params) {
    ...
    const existing = profileVocabularyRows.find((r)=>profileVocabularyRowKey(r.userId, r.vocabularyId, r.lessonId) === key);
...
function countKnownWordsForUser(userId) {
    return getProfileVocabularyForUser(userId).filter((r)=>r.status === 'known').length;
}
...
const toProfileViewModel = (userId)=>{
    ...
    const teacher = user.teacherId ? __TURBOPACK__imported__module__$5b$project$5d2f$apps$2f$web$2f$src$2f$mocks$2f$domains$2f$entities$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["mockUsers"].find((entry)=>entry.id === user.teacherId) : undefined;
    ...
    return {
        ...
        teacherName: user.teacherId ? teacher?.fullName ?? 'Unknown teacher' : '',
...
//# sourceMappingURL=_94db3eee._.js.map