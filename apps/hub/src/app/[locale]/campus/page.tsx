...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const [product, brandKit, site] = await Promise.all([
    getProductBySlug('campus', locale),
    getBrandKit(),
  ...
  return buildHubMetadata({
    ...
    path: '/campus',
    locale,
    languages: hreflangAlternates('/campus'),
  });
}

export default async function CampusLandingPage({ params }: Props) {
  const { locale: raw } = await params;
  ...
  const ctaUrl = resolveProductCtaUrl({
    ...
    locale,
    utm: { utm_campaign: 'campus_landing' },
  });
  ...
  const crumbName =
    product.breadcrumbLabel?.trim() || product.seoTitle?.trim() || product.name || 'Campus';
  ...
  return (
    <>
      ...
      <div className={styles.productLanding} style={style}>
        <p className={styles.eyebrow}>Campus</p>
        {logoUrl ? (
          <CmsLogo
            className={styles.productLogo}
            src={logoUrl}
            alt={mediaAlt(product.logo ?? product.icon, product.name ?? 'Campus')}
            width={176}
            height={44}
            priority
          />
        ...
        <div className={styles.actions}>
          {ctaUrl ? (
            <Link className={styles.button} href={ctaUrl}>
              {product.ctaLabel || (locale === 'uk' ? 'Відкрити Campus' : 'Open Campus')}
            </Link>
          ) : null}
          <Link className={`${styles.button} ${styles.buttonGhost}`} href={`/${locale}`}>
            {locale === 'uk' ? 'На головну' : 'Back home'}
          </Link>
        </div>
      </div>
    </>
