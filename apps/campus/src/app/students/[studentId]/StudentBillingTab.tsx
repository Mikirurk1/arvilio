...
export function StudentBillingTab({
  ...
  const onSaveBillingRules = async () => {
    ...
    if (manualInvoiceEnabledForStudent && platformManualInvoiceMethods.length > 0 && selectedManualMethodIds.length === 0) {
      setFeedback({ action: 'billingRules', kind: 'error', text: 'Select at least one manual invoice template for Manual invoice.' });
      return;
    }
    ...
    try {
      ...
      setPaymentMethodSelection(data.paymentMethodSelection);
      setManualInvoiceSelection(data.manualInvoiceSelection);
      setFeedback({ action: 'billingRules', kind: 'success', text: 'Billing rules saved.' });
    } catch (err) {
      setFeedback({ action: 'billingRules', kind: 'error', text: err instanceof Error ? err.message : 'Save billing rules failed' });
    } finally {
  ...
  const onSavePricing = async () => {
    ...
    try {
      ...
      if (trimmed !== '' && !Number.isFinite(pricePerLessonMinor)) {
        setFeedback({ action: 'pricing', kind: 'error', text: 'Enter a valid price per lesson or leave the field empty.' });
        return;
      }
      ...
      if (groupTrimmed !== '' && !Number.isFinite(groupPricePerLessonMinor)) {
        setFeedback({ action: 'pricing', kind: 'error', text: 'Enter a valid group price per lesson or leave the field empty.' });
        return;
      }
      ...
      setFeedback({ action: 'pricing', kind: 'error', text: err instanceof Error ? err.message : 'Save pricing failed' });
    } finally {
      setPendingAction(null);
    }
  };
  ...
  const onAdjust = async () => {
    ...
    try {
      ...
      setFeedback({ action: 'manualCredit', kind: 'error', text: err instanceof Error ? err.message : 'Adjust failed' });
    } finally {
      setPendingAction(null);
    }
  };
  ...
  if (loading) {
    return <SurfaceCard className={styles.tabCard}><p className={styles.tabIntro}>Loading billing…</p></SurfaceCard>;
  }
        ...
        <div className={styles.billingWorkspaceHeroMain}>
          ...
          <div>
            ...
            <p className={styles.billingWorkspaceText}>
              Manage how the student pays, which package options they see, and how offline invoice payments are credited into lesson balance.
            </p>
          </div>
        </div>
        ...
        <>
            ...
            <div className={styles.billingTracksGrid}>
              {billingTracks.showIndividual && billingTracks.individual ? (
                <div className={styles.billingTrackCard}>
                  <div className={styles.billingTrackEyebrow}>Individual lessons</div>
                  <div className={styles.billingTrackValue}>{billingTracks.individual.balance} lessons{billingTracks.individual.isDebt ? ' · debt' : ''}</div>
                  <p className={styles.billingTrackHint}>Prepaid credits consumed per completed individual lesson.</p>
                  <p className={styles.billingTrackMeta}>Rate: {hideStudentPricing ? 'Hidden' : `${formatMinor(billingTracks.individual.resolvedPricePerLessonMinor, billingTracks.individual.defaultCurrency)} per lesson`}</p>
                </div>
              ) : null}
              {billingTracks.showGroup && billingTracks.group ? (
                <div className={styles.billingTrackCard}>
                  <div className={styles.billingTrackEyebrow}>Group lessons</div>
                  <div className={styles.billingTrackValue}>{billingTracks.group.groupBalance} group lesson credits{billingTracks.group.isDebt ? ' · debt' : ''}</div>
                  <p className={styles.billingTrackHint}>Per-member groups consume group credits. Fixed-total groups bill via monetary charges on the ledger.</p>
                  {!hideStudentPricing ? <p className={styles.billingTrackMeta}>Rate: {formatMinor(billingTracks.group.resolvedGroupPricePerLessonMinor, billingTracks.group.groupCurrency)} per group lesson (per-member)</p> : null}
          ...
          <div className={styles.billingSummaryGrid}>
            <div className={styles.billingSummaryCard}>
              ...
              <div className={styles.billingSummaryValue}>{balance.balance}{balance.isDebt ? ' · debt' : ''}</div>
              <div className={styles.billingSummaryHint}>Manual credits and lesson consumption change this number.</div>
            </div>
            <div className={styles.billingSummaryCard}>
            ...
            {balance.showPerLessonPricing && !hideStudentPricing ? (
              <div className={styles.billingSummaryCard}>
                ...
                <div className={styles.billingSummaryHint}>{balance.isCustomPrice ? 'Custom student rate' : 'Platform default rate'}</div>
              </div>
            ) : null}
            <div className={styles.billingSummaryCard}>
              ...
              <div className={styles.billingSummaryValue}>{balance.showSelfServePackages ? `${balance.packages.length} active` : 'Hidden'}</div>
              <div className={styles.billingSummaryHint}>Min package size: {balance.minPackageLessons} lessons</div>
            </div>
          </div>
